...
if [ $# -gt 0 ] && [ $# -le 2 ]; then
  release=$1
  case $release in
    "testing") images=("${PRINCIPAL_IMAGE[@]}");;
    *) if [ $# -eq 2 ]; then
         case "$2" in
           "prod") PROD=1;;
           *) echo "ERROR: Second argument has an unknown value. Aborting." && exit 1;;
         esac
       fi
       images=("${ALL_IMAGES[@]}");;
  esac
else
  echo "No release version provided or too many arguments provided. Aborting." && exit 1
fi
...
RUN apt-get update && \\
    apt-get -y install libldap2-dev \\
                       ...
                       mariadb-client
EOF
[ -n "${PROD:-}" ] && cat << EOF >> "php$image_dir/Dockerfile"
COPY common/prod_setup.sh /tmp/prod_setup.sh
RUN /tmp/prod_setup.sh && rm /tmp/prod_setup.sh
EOF
cat << EOF >> "php$image_dir/Dockerfile"
