...
- **`simplerisk-minimal/`** — Microservices-oriented: `php:8.x-apache` base, no bundled database. Runs as non-root `simplerisk` user. Requires an external MySQL instance.
...
- **Pushes** trigger separate workflows to publish to Docker Hub and GitHub Container Registry (GHCR). GHCR images are signed with Cosign/sigstore.
